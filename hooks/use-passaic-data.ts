...
export function usePassaicData() {
  ...
  useEffect(() => {
    async function fetchPassaicData() {
      const supabase = createClient()
      
      console.log('🔍 Fetching Passaic office data...')
      
      // Fetch only Passaic office transactions
      const { data: rawData, error } = await supabase
        .from('transactions')
        .select('Date, Amount, "Insurance Company", Office, Status')
        .eq('Status', 'CLEARED')
        .ilike('Office', '%Passaic%') // Case-insensitive match for Passaic
        .order('Date', { ascending: false })

      if (error) {
        console.error('❌ Error fetching Passaic transactions:', error)
        setLoading(false)
        return
      }
      ...
      console.log('✅ Passaic data loaded:', {
