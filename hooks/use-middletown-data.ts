...
export function useMiddletownData() {
  ...
  useEffect(() => {
    async function fetchMiddletownData() {
      const supabase = createClient()
      
      console.log('🔍 Fetching Middletown office data...')
      
      // Fetch only Middletown office transactions
      const { data: rawData, error } = await supabase
        .from('transactions')
        .select('Date, Amount, "Insurance Company", Office, Status')
        .eq('Status', 'CLEARED')
        .ilike('Office', '%Middletown%') // Case-insensitive match for Middletown
        .order('Date', { ascending: false })

      if (error) {
        console.error('❌ Error fetching Middletown transactions:', error)
        setLoading(false)
        return
      }
      ...
      console.log('✅ Middletown data loaded:', {
