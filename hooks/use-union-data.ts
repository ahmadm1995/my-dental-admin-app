...
export function useUnionData() {
  ...
  useEffect(() => {
    async function fetchUnionData() {
      ...
      console.log('🔍 Fetching Union office data...')
      
      // Fetch only Union office transactions
      const { data: rawData, error } = await supabase
        .from('transactions')
        .select('Date, Amount, "Insurance Company", Office, Status')
        .eq('Status', 'CLEARED')
        .ilike('Office', '%Union%') // Case-insensitive match for Union
        .order('Date', { ascending: false })

      if (error) {
        console.error('❌ Error fetching Union transactions:', error)
        setLoading(false)
        return
      }
