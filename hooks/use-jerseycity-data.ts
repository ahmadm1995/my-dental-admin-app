...
export function useJerseyCityData() {
  ...
  useEffect(() => {
    async function fetchjerseyCityData() {
      const supabase = createClient()
      
      console.log('🔍 Fetching jerseyCity office data...')
      
      // Fetch only jerseyCity office transactions
      const { data: rawData, error } = await supabase
        .from('transactions')
        .select('Date, Amount, "Insurance Company", Office, Status')
        .eq('Status', 'CLEARED')
        .ilike('Office', '%Jersey City%') // Case-insensitive match for jerseyCity
        .order('Date', { ascending: false })

      if (error) {
        console.error('❌ Error fetching jerseyCity transactions:', error)
        setLoading(false)
        return
      }
      ...
      console.log('✅ jerseyCity data loaded:', {
