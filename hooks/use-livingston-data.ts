...
export function useLivingstonData() {
  ...
  useEffect(() => {
    async function fetchLivingstonData() {
      const supabase = createClient()
      
      console.log('🔍 Fetching Livingston office data...')
      
      // Fetch only Livingston office transactions
      const { data: rawData, error } = await supabase
        .from('transactions')
        .select('Date, Amount, "Insurance Company", Office, Status')
        .eq('Status', 'CLEARED')
        .ilike('Office', '%livingston%') // Case-insensitive match for Livingston
        .order('Date', { ascending: false })

      if (error) {
        console.error('❌ Error fetching Livingston transactions:', error)
        setLoading(false)
        return
      }
      ...
      console.log('✅ Livingston data loaded:', {
